...
public class WeatherApp extends Application {
    ...
    public void start(Stage stage) throws Exception {
        RootPane.TITLE = "Weather";
        RootPane.ICON_DESCRIPTION = "fas-cloud-sun";
    ...
    private Node loadContent() {
        try {
            ...
            loader.config().setResolverFactory(uri -> {
                ...
                ctx.setInjections(
                    "model", new WeatherAppModel()
                );
                return new DefaultResolver(ctx);
            });
            return loader.load(Launcher.load("weather/Weather.jui")).root();
